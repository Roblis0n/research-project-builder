...
SKILL_ROOT_FILES = {"SKILL.md", "README.md", "AGENTS.md"}
SKILL_PREFIXES = ("agents/", "assets/", "references/", "scripts/")
...
def git_tracked_entries(source_root: Path) -> list[tuple[str, str]]:
    ...
    for record in completed.stdout.split(b"\0"):
        ...
        entries.append((raw_path.decode("utf-8"), raw_mode.decode("ascii")))
    return entries
...
def validate_source_file(source_root: Path, relative_path: str, git_mode: str) -> Path:
    ...
    if not source_path.exists():
        raise ValueError(f"tracked file is missing from the working tree: {relative_path}")
...
def build_copy_plan(source_root: Path) -> list[tuple[Path, str, str]]:
    tracked_entries = git_tracked_entries(source_root)
    tracked_paths = {relative_path for relative_path, _mode in tracked_entries}
    ...
    for relative_path, git_mode in tracked_entries:
        targets = package_targets(relative_path)
        if not targets:
            continue
        source_path = validate_source_file(source_root, relative_path, git_mode)
        for target in targets:
            plan.append((source_path, target, git_mode))
    return sorted(plan, key=lambda item: item[1])


def _copy_file(source: Path, destination: Path, git_mode: str) -> None:
...
def build_plugin(output: Path, *, source_root: Path = ROOT) -> Path:
    ...
    try:
        for source_path, target, git_mode in copy_plan:
            _copy_file(source_path, staging.joinpath(*PurePosixPath(target).parts), git_mode)
